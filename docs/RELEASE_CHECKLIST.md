...
- `scripts/run_xcode_release_gate.sh` passes the full Release test plan, including UI tests, from a healthy Xcode automation session. Do not waive this for a public release.
...
- Latest visual smoke screenshot: `/tmp/cozytime-release-smoke.png`.
...
- UI tests are written for first launch, quick add, focus lifecycle, countdown creation, habit completion, settings persistence, relaunch persistence, and an accessibility audit smoke test, but the current local session still times out while synthesizing UI events.
